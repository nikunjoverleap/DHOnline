...
- (void)getSDKToken:(NSDictionary *)input completionHandler:(void (^)(bool  Success))completionHandler
{
  ...
  PayFortController *PayFort = [[PayFortController alloc]initWithEnviroment:[isLive boolValue] ? PayFortEnviromentProduction : PayFortEnviromentSandBox];
...
- (void)payWithPayfort:(NSDictionary *)input
{
  ...
  PayFort.isShowResponsePage = YES;
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      [PayFort callPayFortWithRequest:request currentViewController:nav success:^(NSDictionary<NSString *,NSString *> * requestDic, NSDictionary<NSString *,NSString *> * responeDic) {
        NSLog(@"Success");
        NSLog(@"responeDic=%@",responeDic);
        successCallbackPayfort(@[responeDic]);
      } canceled:^(NSDictionary<NSString *,NSString *> * requestDic, NSDictionary<NSString *,NSString *> * responeDic) {
        NSLog(@"Canceled");
        NSLog(@"responeDic=%@",responeDic);
        errorCallbackPayfort(@[responeDic]);
      } faild:^(NSDictionary<NSString *,NSString *> * requestDic, NSDictionary<NSString *,NSString *> * responeDic, NSString * message) {NSLog(@"Faild");
        NSLog(@"responeDic=%@",responeDic);
        errorCallbackPayfort(@[responeDic]);
      }];;
    });
...
- (void)applePayWithPayfort:(RCTResponseSenderBlock )errorCallback andSuccessCallbackApplePay:(RCTResponseSenderBlock )successCallback
{
    ...
    request.merchantIdentifier = [applePayRequestDict objectForKey:@"merchant_identifier_apple"];

        request.supportedNetworks = @[PKPaymentNetworkVisa, PKPaymentNetworkMasterCard];

        request.merchantCapabilities = PKMerchantCapability3DS;
        request.currencyCode =  [applePayRequestDict objectForKey:@"countryCode"];
  ...
  NSLog(@"arrSummaryItems == %@", request);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      [topController presentViewController:applePayController animated:YES completion:nil];
    });
...
//    completion(PKPaymentAuthorizationStatusSuccess);
    ...
    if(asyncSuccessful) {
      ...
      NSLog(@"request-applePay token_name%@", token_name);
      NSLog(@"request-applePay amount%@", amount);
      PayFort.isShowResponsePage = YES;
        
        UIViewController *nav =  (UIViewController*)[UIApplication sharedApplication].delegate.window.rootViewController;
      
      [PayFort callPayFortForApplePayWithRequest:request applePayPayment:payment currentViewController:nav success:^(NSDictionary<NSString *,NSString *> * requestDic, NSDictionary<NSString *,NSString *> * responeDic) {
        isApplePaymentDidPayment = TRUE;
        successCallbackApplePay(@[responeDic]);
        completion(PKPaymentAuthorizationStatusSuccess);
      } faild:^(NSDictionary<NSString *,NSString *> * requestDic, NSDictionary<NSString *,NSString *> * responeDic, NSString * message) {
        isApplePaymentDidPayment = TRUE;
        errorCallbackApplePay(@[responeDic]);
        NSLog(@"response-applePay%@", responeDic);
